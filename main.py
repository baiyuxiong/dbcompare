...
class SQLCompareApp:
    ...
    def create_main_content(self):
        ...
        # 创建左侧表格
        self.left_tree = ttk.Treeview(left_frame, columns=("index", "field"), show="headings")
        ...
        # 创建右侧表格
        self.right_tree = ttk.Treeview(right_frame, columns=("index", "field"), show="headings")
        ...
        # 创建左侧定义文本框
        self.left_def_text = scrolledtext.ScrolledText(left_frame, wrap=tk.WORD, height=10)
        self.left_def_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.left_def_text.tag_configure("diff", foreground="red", font=('DejaVu Sans Mono', 10, 'bold'))
        
        # 创建右侧定义文本框
        self.right_def_text = scrolledtext.ScrolledText(right_frame, wrap=tk.WORD, height=10)
        self.right_def_text.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
        self.right_def_text.tag_configure("diff", foreground="red", font=('DejaVu Sans Mono', 10, 'bold'))
    ...
    def show_differences(self):
        ...
        self.left_def_text.delete('1.0', tk.END)
        self.right_def_text.delete('1.0', tk.END)
        ...
        for table_index, table_name in enumerate(all_tables, 1):
            ...
            # 添加表头
            self.left_tree.insert("", "end", values=(f"表{table_index}", f"表名: {table_name}"), tags=("table_header",))
            self.right_tree.insert("", "end", values=(f"表{table_index}", f"表名: {table_name}"), tags=("table_header",))
            ...
            if all_columns:
                ...
                for col_index, col_name in enumerate(all_columns, 1):
                    ...
                    # 插入行
                    self.left_tree.insert("", "end", values=(col_index, col_name), tags=tuple(left_tags))
                    self.right_tree.insert("", "end", values=(col_index, col_name), tags=tuple(right_tags))
                    
                    # 在文本框中显示定义
                    if left_def_display and right_def_display:
                        # 找出差异部分
                        diffs = self.find_differences(left_def_display, right_def_display)
                        
                        # 为差异部分添加标记
                        left_text = left_def_display
                        right_text = right_def_display
                        
                        # 在左侧文本框中插入文本
                        start_pos = self.left_def_text.index(tk.END)
                        self.left_def_text.insert(tk.END, left_text + "\n")
                        
                        # 为差异部分添加红色标记
                        for tag, i1, i2, j1, j2 in diffs:
                            if tag in ('replace', 'delete'):
                                diff_start = f"{start_pos}+{i1}c"
                                diff_end = f"{start_pos}+{i2}c"
                                self.left_def_text.tag_add("diff", diff_start, diff_end)
                        
                        # 在右侧文本框中插入文本
                        start_pos = self.right_def_text.index(tk.END)
                        self.right_def_text.insert(tk.END, right_text + "\n")
                        
                        # 为差异部分添加红色标记
                        for tag, i1, i2, j1, j2 in diffs:
                            if tag in ('replace', 'insert'):
                                diff_start = f"{start_pos}+{j1}c"
                                diff_end = f"{start_pos}+{j2}c"
                                self.right_def_text.tag_add("diff", diff_start, diff_end)
                    else:
                        # 如果有一方缺失，直接显示
                        self.left_def_text.insert(tk.END, (left_def_display or "[缺失]") + "\n")
                        self.right_def_text.insert(tk.END, (right_def_display or "[缺失]") + "\n")
            ...
            if all_indexes:
                ...
                for idx_index, idx_name in enumerate(all_indexes, 1):
                    ...
                    # 插入行
                    self.left_tree.insert("", "end", values=(idx_index, idx_name), tags=tuple(left_tags))
                    self.right_tree.insert("", "end", values=(idx_index, idx_name), tags=tuple(right_tags))
