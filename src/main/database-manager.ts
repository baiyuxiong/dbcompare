...
import { Connection, History } from './types';
...
export class DatabaseManager {
  ...
  private writeConnections(connections: Connection[]) {
  ...
  private writeHistory(history: History[]) {
  ...
  async addConnection(connection: Connection): Promise<number> {
    const connections = this.readConnections();
  ...
  async getHistory(side: string, limit: number = 10): Promise<History[]> {
    const history = this.readHistory();
  ...
  async updateHistoryLastUsed(id: number): Promise<void> {
    const history = this.readHistory();
